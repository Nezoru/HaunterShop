...
import { fetchCardData, getAllProduk } from '@/app/lib/data';

export default async function HalloweenShop() {
  const {
    numberOfInvoices,
    numberOfCustomers,
    totalPaidInvoices,
  ...
  // Fetch products from the database instead of using hardcoded array
  const products = await getAllProduk();
  ...
  return (
    <div className="min-h-screen bg-black bg-gradient-to-b from-black to-gray-900 text-white pb-20">
        ...
        {products.map((product) => (
          <Link href={`/dashboard/produk-cus/detail?id=${product.id}`} key={product.id}>
            <div className="bg-white rounded-lg overflow-hidden text-center p-4 h-full transition-transform hover:scale-105 cursor-pointer">
              ...
              <h3 className={`${shadowsIntoLightTwo.className} text-2xl md:text-3xl text-black`}>
                {product.nama_produk}
              </h3>
              <p className={`${shadowsIntoLightTwo.className} text-1xl md:text-2xl text-black`}>
      ...
      {/* Guided */}
      {/* <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4 px-12 mt-12">
        <Card title="Kostum Terlaris" value={numberOfInvoices} type="invoices" />
        <Card
          title="Total Produk"
